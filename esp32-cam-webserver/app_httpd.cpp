...
extern void setVertical(int pos);
extern void setHorizontal(int pos);
extern void setSteering(int pos);
extern void controlMotor(int action);
...
static esp_err_t capture_handler(httpd_req_t *req){
    ...
    if (vPos != -1) setVertical(vPos);
    if (hPos != -1) setHorizontal(hPos);
    if (sPos != -1) setSteering(sPos);
    controlMotor(action);
...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    else if(!strcmp(variable, "verticalServoSlider")) {
        vPos = constrain(val,0,90);
        setVertical(vPos);
    }
    else if(!strcmp(variable, "horizontalServoSlider")) {
        hPos = constrain(val,0,180);
        setHorizontal(hPos);
    }
    else if(!strcmp(variable, "steeringServoSlider")) {
        sPos = constrain(val,0,360);
        setSteering(sPos);
    }
    else if(!strcmp(variable, "motorController")) {
        action = constrain(val, -1, 1);
        controlMotor(action);
    }
...
static esp_err_t status_handler(httpd_req_t *req){
    ...
    *p++ = '{';
    ...
    if (critERR.length() == 0) {
        ...
        p+=sprintf(p, "\"vertical\":%d,", vPos);
        p+=sprintf(p, "\"horizontal\":%d,", hPos);
        p+=sprintf(p, "\"steering\":%d,", sPos);
        p+=sprintf(p, "\"moving\":%d,", action);
