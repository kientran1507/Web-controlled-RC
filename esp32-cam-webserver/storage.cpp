...
extern int vPos;          // The current vertical servo position
extern int hPos;          // The current horizontal servo position
...
void loadPrefs(fs::FS &fs){
  if (fs.exists(PREFERENCES_FILE)) {
    ...
    if (vPos >= 0) {
        int vPosPref = jsonExtract(prefs, "vertical").toInt();
        if (vPosPref >= 0) vPos = vPosPref;
    }
    if (hPos >= 0) {
        int hPosPref = jsonExtract(prefs, "horizontal").toInt();
        if (hPosPref >= 0) hPos = hPosPref;
    }
...
void savePrefs(fs::FS &fs){
  ...
  *p++ = '{';
  ...
  p+=sprintf(p, "\"vertical\":%i,", vPos);
  p+=sprintf(p, "\"horizontal\":%i,", hPos);
